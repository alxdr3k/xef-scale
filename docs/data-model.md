...
| `Transaction` | `transactions` | workspace_id, date, amount, merchant, description, category_id, financial_institution_id, parsing_session_id, status, payment_type, source_type, classification_source, installment_*, deleted, parse_confidence, source_metadata | `belongs_to :workspace, :category?, :financial_institution? (legacy optional), :parsing_session?, :committed_by(User)?`. `classification_source`(ADR-0011): 결정 메커니즘 4값 `manual_set` / `mapping_match` / `keyword_match` / `gemini_batch`, nullable. `CategoryMapping#source`(매핑 출처)와 *축이 다름* — ADR-0007 §1 참조. |
