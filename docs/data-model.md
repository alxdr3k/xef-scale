...
이 문서는 위에서 *현재* 추출한 형태이며, 새 마이그레이션이 들어오면 같은 PR에서 갱신해야 합니다 (`docs/DOCUMENTATION.md` 참조).
...
| `Workspace` | `workspaces` | name, owner_id, ai_text_parsing_enabled, ai_image_parsing_enabled, ai_category_suggestions_enabled, ai_consent_acknowledged_at | owner는 `User`. `has_many :memberships, transactions, categories, category_mappings, parsing_sessions, processed_files, api_keys`; `has_one :budget` |
...
| `Transaction` | `transactions` | workspace_id, date, amount, merchant, description, category_id, financial_institution_id, parsing_session_id, status, payment_type, source_type, installment_*, deleted, parse_confidence | `belongs_to :workspace, :category?, :financial_institution?, :parsing_session?, :committed_by(User)?` |
...
| `ProcessedFile` | `processed_files` | workspace_id, uploaded_by_id, filename, original_filename, file_hash, status | `has_one :parsing_session`, `has_one_attached :file` |
...
| `Budget` | `budgets` | workspace_id, monthly_amount | 워크스페이스당 1개 |
...
`FinancialInstitution`은 워크스페이스 스코프가 아닌 글로벌 시드 데이터.
