...
| `UX-1B.4` | `P1-M1` | `UX` | `UX-1B` | Provide an import-level undo/recovery path that replaces review discard/rollback for auto-posted imports. | `INP-1B.1` | undo/recovery tests + user copy review | `defined` | `ready` | [ADR-0001](decisions/ADR-0001-auto-post-imports.md); `INP-1B.1`, `UX-1B.3`, and `INP-1B.4` accepted | Required before deleting review routes; repair editing/promotion is now on `dev`, so this is the next safety gate. |
