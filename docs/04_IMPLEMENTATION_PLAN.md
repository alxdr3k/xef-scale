...
| `P1-M1` | Founder/wife can use the mobile web flow end-to-end without engineering help and the team has observation evidence. | next | `ready` | `ROAD-001` |  | First post-ledger execution milestone. |
...
| `UX` | Mobile web UX, family onboarding, input ergonomics | `UX-1A` | `ready` | First user-observation milestone. |
...
| `INS` | Dashboard, budget, recurring payments, insights | `INS-1A` | `planned` | Some code exists; acceptance and product polish remain. |
...
| `DOC-1C.1` | `P0-M2` | `DOC` | `DOC-1C` | Expand implementation plan into full leaf roadmap from PRD/HLD/design docs. | user request | markdown link review | `not_run` | `in_progress` | pending PR | Validate and publish. |
...
| `UX-1A.2` | `P1-M1` | `UX` | `UX-1A` | Audit and fix mobile navigation so dashboard, transactions, upload, and settings are reachable on phones. | `UX-1A.1` | manual mobile check + e2e if changed | `defined` | `planned` | design-phase-a Task 3 |  |
...
| `INS-1A.1` | `P1-M1` | `INS` | `INS-1A` | Audit budget setting, progress display, 80%/100% alert behavior, and tests; add missing acceptance if needed. | `P0-M0` | budget manual/automated check | `defined` | `planned` | design-phase-a Task 7, current Budget model |  |
| `INS-1A.2` | `P1-M1` | `INS` | `INS-1A` | Audit recurring-payment detection/list UX, false positives, and test coverage. | `P0-M0` | recurring check | `defined` | `planned` | design-phase-a Task 8, current service |  |
| `INS-1A.3` | `P1-M1` | `INS` | `INS-1A` | Improve mobile dashboard hierarchy: monthly total hero, category percentages, family-readable summaries. | `UX-1A.2` | visual/manual check + e2e if changed | `defined` | `planned` | design-phase-a Task 3 |  |
