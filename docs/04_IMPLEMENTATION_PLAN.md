...
| `P1-M1` | Founder/wife can use the mobile web flow end-to-end without engineering help and the team has observation evidence. | next | `in_progress` | `ROAD-001` | `UX-1A.2` mobile nav reachability patch; `INS-1A.1` budget setting/progress/alert audit | First post-ledger execution milestone; human observation remains the main gate. |
...
| `INS` | Dashboard, budget, recurring payments, insights | `INS-1A` | `in_progress` | Budget setting/progress/alert audit accepted; recurring-payment polish remains. |
