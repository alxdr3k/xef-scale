...
| #240 jwt 3.1.2 → 3.2.0 보안 업데이트 | GPT review on #240 chain (F4) | ✅ fixed | dependabot PR #240 머지 완료 (CVE-2026-45363 / GHSA-c32j-vqhx-rx3x — nil/empty HMAC key 거부). lockfile-only diff. |
...
| i18n contract — user-visible model methods targeted audit | GPT review (F7) | 🟡 partial-deferred | user-visible model methods 만 targeted audit (다음 i18n 작업 때 같이 반영); 전체 model surface 는 false positive 폭증으로 defer. "model i18n 영구 기각" 이 아니라 surface 를 분할한 결정임을 명확히 함. |
