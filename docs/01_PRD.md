...
Rails 기반 지출 추적 앱. 한국 금융기관의 거래 내역을 워크스페이스 단위로 입력, 자동 등록, 예외 수정, 분류, 조회한다.

이 문서는 제품 계약을 `REQ-###` / `NFR-###` 단위로 정리한다. 구현 세부는 [02_HLD.md](02_HLD.md)와 [current/](current/)를 따른다.

> P1 전환 계약: [ADR-0001](decisions/ADR-0001-auto-post-imports.md)에 따라 텍스트/이미지 파싱의 목표 UX는 mandatory review가 아니라 auto-post + repair-only exceptions다. 현 코드는 complete row auto-post, exact duplicate skip, ambiguous duplicate repair issue, incomplete image row repair record persistence, repair surfacing을 구현했고, 남은 repair editing/promotion, undo/recovery, review 제거 작업은 [04_IMPLEMENTATION_PLAN.md](04_IMPLEMENTATION_PLAN.md)의 `INP-1B` / `UX-1B` slice로 추적한다.

## Problem

한국 금융기관의 카드/은행 지출 내역은 SMS, 앱 화면, 카드 명세서 등 여러 표면에 흩어져 있다. 사용자는 이를 빠르게 거래 장부로 모으고, 가족/팀 워크스페이스에서 분류·조회할 수 있어야 한다. 정상적으로 파싱된 거래는 사용자를 멈춰 세우지 않고 장부에 들어가야 하며, 사용자는 누락 필수값·애매한 중복처럼 실제로 개입이 필요한 예외만 처리해야 한다.

## Users & Goals

- 주요 사용자: 개인, 가족, 소규모 팀/워크스페이스의 지출 관리 사용자.
- 사용자 목표: 직접 입력 또는 AI 파싱으로 거래를 빠르게 등록하고, 누락 필수값·애매한 중복만 수정한 뒤 월별/연별 지출을 조회한다.
...
- 파싱 성공 결과 자동 등록, 예외 repair queue, 중복 자동 처리/수정 요청, import-level undo/recovery.
...
| REQ-002 | 사용자는 금융 문자/SMS 텍스트를 붙여넣어 AI 파싱 세션을 만들 수 있고, 필수값이 완성된 결과 거래는 사용자 검토 없이 즉시 `committed` 상태로 저장된다. | must | AC-002 |
| REQ-003 | 사용자는 허용된 이미지 형식의 명세서 스크린샷을 업로드해 AI 파싱 세션을 만들 수 있고, 필수값이 완성된 결과 거래는 즉시 `committed` 상태로 저장되며 필수값이 부족한 행은 repair queue에 보존된다. | must | AC-003 |
| REQ-004 | API key 사용자는 `read` 스코프로 거래/summary를 조회하고, `write` 스코프로 거래를 생성할 수 있다. API write 거래는 즉시 `committed` 상태로 저장된다. | must | AC-004 |
| REQ-005 | 사용자는 파싱 세션 전체를 매번 검토하지 않고, 수정이 필요한 import repair item만 열어 누락 필수값을 채우거나 제외할 수 있으며, 완료된 repair item은 `committed` 거래로 승격된다. | must | AC-005 |
| REQ-006 | 자동 등록 흐름은 명확한 중복을 장부에 중복 반영하지 않아야 하며, 애매한 중복은 `committed` 거래가 아니라 사용자가 판단할 repair issue로 남겨야 한다. | must | AC-006 |
...
- 중복 의심/자동 제외 비율: duplicate issue or skipped duplicate count / parsed count.
- 미분류 → Gemini 카테고리 추천 적중률.
- 자동 등록 성공/실패 수와 repair item 해소율.
