...
> P1 전환 계약: [ADR-0001](decisions/ADR-0001-auto-post-imports.md)에 따라 텍스트/이미지 파싱의 목표 UX는 mandatory review가 아니라 auto-post + repair-only exceptions다. 현 코드는 non-duplicate complete row auto-post를 구현했고, 남은 duplicate/repair/undo/review 제거 작업은 [04_IMPLEMENTATION_PLAN.md](04_IMPLEMENTATION_PLAN.md)의 `INP-1B` / `UX-1B` slice로 추적한다.
