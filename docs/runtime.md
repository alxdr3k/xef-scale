...
7. 통계 hash로 분기:
   - `stats[:total] > 0` → `parsing_session.complete!(stats)` + `processed_file.mark_completed!` + `create_completion_notifications`.
   - `stats[:total].zero?` → `parsing_session.fail!` + `processed_file.mark_failed!` + `create_failure_notifications` (이 분기에서만 실패 인앱 알림이 발송된다).
   - 그 외 예외(Vision 호출/저장 등) → 잡 상단의 `rescue => e`가 `parsing_session.fail!` + `processed_file.mark_failed!`만 실행하고 알림은 보내지 않는다.
...
| Gemini 텍스트 모델 전체 실패 | `AiTextParser#parse`가 `{ transactions: [], model_used: nil }` 반환 → `AiTextParsingJob`이 0건으로 처리해 `parsing_session.fail!`. 잡은 정상 종료. **실패 알림은 발송되지 않는다** (`AiTextParsingJob`에는 `create_failure_notifications`가 없음). |
| Gemini Vision 실패 (예외 raise) | `GeminiVisionParserService`가 `ApiError` raise → `FileParsingJob`의 `rescue => e`가 잡아 `parsing_session.fail!` + `processed_file.mark_failed!`. **이 경로는 `create_failure_notifications`를 호출하지 않으므로 인앱 알림이 발송되지 않는다.** 잡은 정상 종료. |
...
- **파싱 실패 알림** — `FileParsingJob`에서 stats가 0건일 때만 `create_failure_notifications`가 호출되어 owner + `co_owner` / `member_write` 멤버에게 `Notification.create_parsing_failed!`을 발송한다. **`FileParsingJob`의 `rescue => e` 예외 경로와 `AiTextParsingJob`의 모든 실패 경로에서는 실패 알림을 발송하지 않는다** — 운영자는 세션 status가 `failed`로 전환되는 것으로만 인지한다 (`needs follow-up`: 텍스트 경로 실패도 알림 보내야 하는지 결정).
