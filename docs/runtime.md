...
| 상황 | 동작 |
|------|------|
| `GEMINI_API_KEY` 미설정 | 모든 AI 서비스가 `ArgumentError`로 즉시 실패. `FileParsingJob`은 이를 `ArgumentError` rescue로 잡아 카테고리 0건만 반환하고 파싱 자체는 진행. `AiTextParsingJob`은 `AiTextParser.new`에서 raise되어 잡 자체가 실패. |
| Gemini 텍스트 모델 전체 실패 | `AiTextParser#parse`가 `{ transactions: [], model_used: nil }` 반환 → `AiTextParsingJob`이 0건으로 처리해 `parsing_session.fail!`. |
| Gemini Vision 실패 | `GeminiVisionParserService`가 `ApiError` raise → `FileParsingJob`이 rescue로 세션 fail + 파일 mark_failed + `create_failure_notifications` (구현 위치는 `Notification.create_parsing_failed!`). |
...
- 커밋된 거래는 `Notification.create_parsing_complete!`로 멤버에게 알림.
...
자세한 알림 클래스 메서드는 `app/models/notification.rb` (`needs audit` — 본 PR에서는 검사하지 않음).
