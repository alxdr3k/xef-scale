...
- active track / phase / slice: `UX` / `UX-1A` / next recommended external slice `UX-1A.1`; next local implementation candidate `INS-1A.2`.
- last accepted gate: `INS-1A.1` budget setting/progress/80%/100% alert audit passed with controller/model coverage.
