...
- **`Pundit::Authorization` include의 의도** — 모듈은 mount 됐지만 정책 디렉토리가 없음. 의도된 부분 도입인지, 잔재인지 확인 필요.
...
- **`Notification.create_parsing_failed!` / `create_parsing_complete!`** — `Notification` 모델 자체가 존재하지만 이 클래스 메서드의 위치/구현은 본 문서에서 추적하지 않았음.
