...
- `AiTextParsingJob` — rescue 안함 → 잡 자체가 실패하고 세션 fail.
- `FileParsingJob` — `categorize_with_gemini_batch`만 `ArgumentError`/`StandardError` rescue로 카테고리 0건 처리. Vision 호출은 동일 잡 내부 `rescue => e`가 잡아 세션 fail.
...
| 일괄 가져오기 (Rake 태스크) | `CategoryMapping(source: "import")` — 단일 호출: `lib/tasks/import.rake :category_mappings` |
