...
| AC-002 | REQ-002 | Given AI text parsing is enabled and consented, When pasted text is parsed and required fields are complete, Then transactions are created as `committed` with `source_type: text_paste`, source metadata stays metadata, and no mandatory review action is required. | planned TEST-002 | defined |
| AC-003 | REQ-003 | Given AI image parsing is enabled and consented, When an allowed image is uploaded, Then complete rows are created as `committed` transactions and rows missing date/merchant/amount are stored as repair items instead of being guessed or dropped. | planned TEST-003 | defined |
...
| AC-005 | REQ-005 | Given import repair items exist, When the user opens any repair entry point, Then only rows needing action are shown; filling required fields promotes an item to a committed transaction, and excluding an item removes it from the active repair queue. | planned TEST-005 | defined |
| AC-006 | REQ-006 | Given parsed rows match existing transactions, When the import finalizer runs, Then exact duplicates are not double-counted and ambiguous duplicates are held as repair issues rather than committed ledger rows. | planned TEST-005, TEST-006 | defined |
...
| TEST-003 | Image upload/session/job auto-post and repair behavior | `test/controllers/parsing_sessions_controller_test.rb`, `test/jobs/file_parsing_job_test.rb` | AC-003, AC-010 |
...
| TEST-005 | Import repair and undo/recovery flow | `test/controllers/reviews_controller_test.rb`, `test/integration/parsing_review_flow_test.rb` until replaced by repair flow tests | AC-005, AC-006 |
| TEST-006 | Duplicate detector and import duplicate policy behavior | `test/services/duplicate_detector_test.rb`, `test/jobs/file_parsing_job_test.rb` | AC-006 |
