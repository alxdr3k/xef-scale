...
| AC-005 | REQ-005 | Given import repair items exist, When the user opens any repair entry point, Then only rows needing action are shown; filling required fields promotes an item to a committed transaction, and excluding an item removes it from the active repair queue. | partial automated TEST-005; promotion planned | defined |
...
| TEST-005 | Import repair and undo/recovery flow | `test/controllers/transactions_controller_test.rb` covers repair entry surfacing/filtering; `test/controllers/reviews_controller_test.rb`, `test/integration/parsing_review_flow_test.rb` remain until replaced by repair flow tests | AC-005, AC-006 |
