...
| AC-002 | REQ-002 | Given AI text parsing is enabled and consented, When pasted text is parsed, Then transactions are created as `pending_review` with `source_type: text_paste` and source metadata stays metadata. | automated TEST-002 | passing |
| AC-003 | REQ-003 | Given AI image parsing is enabled and consented, When an allowed image is uploaded, Then a processed file/session is created and the file parsing job creates pending review transactions. | automated TEST-003 covers upload/session behavior; pending review transaction creation assertion gap | not_run |
...
| AC-005 | REQ-005 | Given a completed parsing session, When the user commits, rolls back, or discards it, Then transaction statuses and existing originals are updated according to the review action. | automated TEST-005 | passing |
| AC-006 | REQ-006 | Given pending duplicate confirmations, When commit is requested, Then commit is blocked until duplicates are resolved; resolved duplicate decisions are honored. | automated TEST-005, TEST-006 | passing |
| AC-007 | REQ-007 | Given transactions with merchants/descriptions, When category matching runs, Then mapping and keyword matches are deterministic; image path can batch Gemini fallback, text path does not call Gemini category fallback. | automated TEST-007 covers mapping/keyword behavior; Gemini fallback decision-boundary assertions gap | not_run |
