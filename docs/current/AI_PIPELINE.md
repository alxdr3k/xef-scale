...
이미지 경로에서 날짜/가맹점/금액 중 필수 정보가 보이지 않는 행은 자동으로 날짜를 추정하지 않는다. Gemini 응답은 보이는 필드만 포함한 `incomplete_transactions`를 반환할 수 있고, `FileParsingJob`은 해당 행을 결제 내역으로 만들지 않고 세션 오류/노트로 남겨 검토 화면에 노출한다.
