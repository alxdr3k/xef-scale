...
| Path | Purpose | Status |
|------|---------|--------|
| [app/controllers/transactions_controller.rb](../../app/controllers/transactions_controller.rb) | 직접 입력 (`new`/`create`)·인라인 편집·CSV 내보내기·import repair banner/filter·import session scoped ledger filter | implemented |
| [app/controllers/import_issues_controller.rb](../../app/controllers/import_issues_controller.rb) | import repair item 누락값 저장, 거래 승격, 중복 판단 처리 | implemented |
| [app/controllers/parsing_sessions_controller.rb](../../app/controllers/parsing_sessions_controller.rb) | 텍스트 붙여넣기 (`text_parse`)·이미지 업로드 (`create`)·세션 폐기 (`bulk_discard`)·입력 기록 import undo (`undo`) | implemented |
...
| [app/controllers/pages_controller.rb](../../app/controllers/pages_controller.rb) | 비로그인 landing/root 페이지 | implemented |
...
| [app/models/parsing_session.rb](../../app/models/parsing_session.rb) | 파싱 세션 + import batch 상태 머신 (auto-commit/undo/legacy review commit/rollback/discard) | implemented |
| [app/models/processed_file.rb](../../app/models/processed_file.rb) | 업로드 파일 — 이미지 검증 + 매직 바이트 sniff | implemented |
| [app/models/import_issue.rb](../../app/models/import_issue.rb) | 필수값 누락·ambiguous duplicate import repair record | implemented |
| [app/models/duplicate_confirmation.rb](../../app/models/duplicate_confirmation.rb) | legacy review 중복 후보·결정 | implemented |
| [app/models/notification.rb](../../app/models/notification.rb) | 인앱 알림 + import repair toast broadcast | implemented |
...
| [app/services/import_duplicate_policy.rb](../../app/services/import_duplicate_policy.rb) | import 중복 정책: exact duplicate skip, ambiguous duplicate `ImportIssue` 생성 | implemented |
| [app/services/import_repair_notifier.rb](../../app/services/import_repair_notifier.rb) | open `ImportIssue` 수신자(owner/co_owner/member_write) repair 알림 발송 | implemented |
| [app/services/import_issue_resolution_service.rb](../../app/services/import_issue_resolution_service.rb) | repair item을 committed 거래로 승격하거나 exact/ambiguous duplicate 및 제외 처리 | implemented |
| [app/services/budget_alert_service.rb](../../app/services/budget_alert_service.rb) | 월별 예산 80%/100% 임계값 알림 (전체 멤버 대상, 중복 발송 방지) | implemented |
| [app/services/recurring_payment_detector.rb](../../app/services/recurring_payment_detector.rb) | 반복 결제 패턴 탐지 (대시보드용) | implemented |
| [app/services/database_backup_service.rb](../../app/services/database_backup_service.rb) | SQLite 백업 헬퍼 | implemented (development/import-only; environment guard; 운영 백업 아님) |
| [app/services/sqlite_backup_service.rb](../../app/services/sqlite_backup_service.rb) | configured SQLite role backup/restore primitive | implemented (SQLite backup API + checkpoint + integrity_check; 운영 보관/스케줄 정책 없음) |

## 잡

| Path | Purpose | Status |
|------|---------|--------|
| [app/jobs/ai_text_parsing_job.rb](../../app/jobs/ai_text_parsing_job.rb) | 텍스트 붙여넣기 비동기 파싱 + 매핑/keyword 카테고리화 + import duplicate policy + 자동 커밋/repair 알림 | implemented |
| [app/jobs/file_parsing_job.rb](../../app/jobs/file_parsing_job.rb) | 이미지 비동기 파싱 + incomplete/duplicate `ImportIssue` 저장/repair 알림 + 3단계 카테고리화 + 자동 커밋 | implemented |
...
| [app/views/transactions/](../../app/views/transactions/) | 거래 인덱스/폼/turbo_stream 응답/import repair filter |
| [app/views/notifications/](../../app/views/notifications/) | 알림 목록/드롭다운 |
| [app/views/shared/_toast.html.erb](../../app/views/shared/_toast.html.erb) | flash와 async notification toast 공용 partial |
...
| [package.json](../../package.json), [bun.lock](../../bun.lock), [yarn.lock](../../yarn.lock) | JS 의존성 manifest + Bun/Yarn lockfiles |

JS 번들링: esbuild. CSS: tailwind CLI. 로컬 시작은 `bin/dev` (Procfile.dev/Bun 사용), Rails asset prep와 Docker 빌드는 Yarn lock 경로도 사용한다.
...
| Path | Purpose | Status |
|------|---------|--------|
| [lib/tasks/import.rake](../../lib/tasks/import.rake) | 일괄 가져오기 — development/import 전용 DB snapshot → 카테고리 매핑(`source: "import"`) → 거래 import. `DatabaseBackupService` 단일 호출 지점. | implemented |
...
| Path | Purpose |
|------|---------|
| [.github/workflows/ci.yml](../../.github/workflows/ci.yml) | brakeman, bundler-audit, rubocop, rails test, system-test, Playwright e2e |
...
| [app/services/database_backup_service.rb](../../app/services/database_backup_service.rb) | development/test 환경의 current primary DB 단순 복사 전용 — STG/PRD 백업/복구 프로세스가 아님 |
| [app/services/sqlite_backup_service.rb](../../app/services/sqlite_backup_service.rb) | 검증된 SQLite 파일 primitive만 제공 — STG/PRD 스케줄, off-server 보관, 보존 정책, 복구 리허설은 Q-008 필요 |
