...
7. 통계 hash로 분기:
   ...
   - `stats[:total].zero?` 또는 `stats[:success].zero?` → `parsing_session.fail!` + `processed_file.mark_failed!` + `create_failure_notifications`.
...
- **반복 결제 탐지** — `DashboardsController#recurring`이 `RecurringPaymentDetector#detect`를 호출한다. detector는 active(committed, non-deleted) + non-coupon 거래 중 같은 가맹점이 2개월 이상 월간 연속성을 보이고, 금액이 20% 이내로 안정적이거나 결제일 변동 폭이 7일 이내인 경우만 반복 결제로 표시한다.
