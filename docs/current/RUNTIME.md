...
### 3.3 Legacy review → 커밋 (전환기 예외)

1. 사용자가 `/workspaces/:id/parsing_sessions/:id/review` 페이지를 열어 아직 pending으로 남은 전환기 거래를 확인할 수 있다. 새 text/image import duplicate policy는 `DuplicateConfirmation`을 만들지 않지만, legacy 데이터와 일부 non-complete staging row를 위해 review route는 아직 남아 있다. Focused repair entry point는 결제 내역의 `repair=required` 모드이며, writer는 여기서 누락 필수값을 채워 `ImportIssue`를 committed `Transaction`으로 승격하거나, ambiguous duplicate를 제외/새 거래 등록으로 처리한다.
