...
| Gemini Vision 실패 (예외 raise) | `GeminiVisionParserService`가 `ApiError` raise → `FileParsingJob`의 `rescue => e`가 잡아 `parsing_session.fail!` + `processed_file.mark_failed!` + `create_failure_notifications(parsing_session) if parsing_session`. 잡은 정상 종료. |
