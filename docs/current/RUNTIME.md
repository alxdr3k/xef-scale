...
이미지 업로드 ──► FileParsingJob ──► ImageStatementParser ─────────┤
                                     └► GeminiVisionParserService   │
                                                                    ...
                                                                    ├─► ambiguous duplicate ─► ImportIssue repair record
                                                                    ├─► incomplete image row ─► ImportIssue repair record
                                                                    │
API write ──────────────────────────────────────────────────────────┴─► committed Transaction (review skip)
```

전환 목표: [ADR-0001](../decisions/ADR-0001-auto-post-imports.md)은 텍스트/이미지 파싱 결과 중 complete row를 파싱 완료 시점에 자동 `committed`로 저장하고, 필수값 누락·애매한 중복만 repair queue로 보내도록 결정했다. 현재 코드는 complete row 자동 커밋, exact duplicate skip, ambiguous duplicate `ImportIssue(status: open)`, incomplete image row `ImportIssue(status: open)` 저장, transaction repair mode에서의 누락값 입력/중복 판단/거래 승격을 구현했다.
...
4. 각 거래 생성 후 `ImportDuplicatePolicy`가 중복을 처리한다.
   - 같은 import 세션 안의 exact duplicate 또는 기존 committed 거래와 date/amount/normalized merchant/payment type/installment/original amount가 같은 exact duplicate는 staging row를 삭제하고 `duplicate_count`만 증가시킨다.
   - `DuplicateDetector.new(workspace, transaction).find_match`가 반환한 match 중 exact가 아닌 항목은 `ImportIssue(issue_type: "ambiguous_duplicate", duplicate_transaction_id: <기존 거래>)`로 저장하고 staging row를 삭제한다.
5. 결과가 0건이거나, 성공 row/repair issue 없이 저장 에러만 남으면 `fail!` + `create_failure_notifications`. Exact duplicate skip만 있고 저장 에러도 있으면 사용자가 retry할 수 있도록 실패로 처리한다.
6. outcome이 있으면 `ParsingSession#auto_commit_ready_transactions!`가 pending row 중 날짜/가맹점/금액이 모두 있고 repair issue가 아닌 row를 `Transaction#commit!`으로 전환한다. 모든 pending row가 처리되고 import exception이 없으면 세션 `review_status`도 `committed`가 된다.
7. 자동 커밋된 거래 월에 대해 `BudgetAlertService.create_for_transactions!`가 예산 알림을 생성하고, `parsing_session.complete!(stats)` 후 `Notification.create_parsing_complete!`로 워크스페이스 owner + (`co_owner` / `member_write` / `member_read`)에게 알림. Completion action은 해당 import의 scoped ledger(`/transactions?import_session_id=...`)로 이동하며, 성공 row 없이 repair issue만 있으면 repair queue(`/transactions?repair=required&import_session_id=...`)로 이동한다. Open `ImportIssue`가 있으면 owner + `co_owner` / `member_write`에게 별도 repair 알림도 보낸다. 이 post-commit side effect는 best-effort이며, 실패해도 이미 커밋된 거래와 completed 세션을 failed로 되돌리지 않는다. 사용자가 입력 기록에서 해당 import를 되돌리면 committed row는 `rolled_back`이 되고 open repair 알림은 read 처리된다.
...
2. `parse_file`:
   ...
   - 날짜/가맹점/금액 중 필수 정보가 부족한 행은 `incomplete_transactions`로 보존한다. `FileParsingJob`은 이 행을 `Transaction`으로 만들지 않고 `ImportIssue(issue_type: "missing_required_fields")`로 저장한다. `ImportIssue`는 보이는 `date`/`merchant`/`amount`, `missing_fields`, 원본 `parsing_session`, `processed_file`, `source_type`, `raw_payload`, `status`를 가진다. 전부 incomplete라 저장 가능한 거래가 없어도 repair record는 남고 세션/파일은 completed 상태로 repair 알림을 보낸다.
...
5. `ImportDuplicatePolicy` → exact duplicate는 skip, ambiguous duplicate는 `ImportIssue(issue_type: "ambiguous_duplicate")`로 전환 (텍스트 경로와 동일).
6. **Gemini 카테고리 폴백**:
...
7. 통계 hash로 분기:
   - `stats[:total] > 0`이고 저장·중복·repair issue 중 outcome이 있으면 `ParsingSession#auto_commit_ready_transactions!(user: processed_file.uploaded_by)`로 repair issue가 아닌 row를 자동 `committed` 전환하고, `parsing_session.complete!(stats)` + `processed_file.mark_completed!` 후 best-effort로 `BudgetAlertService.create_for_transactions!` + `create_completion_notifications` + `create_import_repair_notifications`를 실행한다. Open `ImportIssue`가 있으면 owner / `co_owner` / `member_write`에게 `import_repair_needed` 알림이 생성되고, 열려 있는 세션에는 warning toast가 broadcast된다.
   - `stats[:total].zero?`, outcome이 전혀 없음, 또는 성공 row/repair issue 없이 저장 에러만 남음 → count를 세션에 저장한 뒤 `parsing_session.fail!` + `processed_file.mark_failed!` + `create_failure_notifications`.
...
### 3.3 입력 기록 undo / Legacy review 전환기 예외

1. 입력 기록(`/workspaces/:id/parsing_sessions`)의 completed row action은 user-facing mandatory review로 보내지 않는다. Open `ImportIssue`가 있으면 `수정하기`가 transaction repair mode로 이동하고, committed row가 있으면 `결제 내역`이 `/transactions?import_session_id=<session>` scoped ledger로 이동한다. Legacy-only row는 route 제거 전까지 `상세보기`로 review detail에 접근할 수 있다.
2. 입력 기록의 writer-only `되돌리기` 액션은 `ParsingSessionsController#undo` → `ParsingSession#undo_import!`를 호출한다. 같은 import의 committed row는 `rolled_back`, pending row는 destroy, open `ImportIssue`는 `dismissed`로 전환하고 repair notification은 read 처리한다. committed row가 하나라도 되돌아가면 세션은 `review_status: rolled_back`, repair/pending-only import는 `discarded`가 된다.
3. 사용자가 `/workspaces/:id/parsing_sessions/:id/review` 페이지를 열어 아직 pending으로 남은 전환기 거래를 확인할 수 있다. 새 text/image import duplicate policy는 `DuplicateConfirmation`을 만들지 않지만, legacy 데이터와 일부 non-complete staging row를 위해 review route는 아직 남아 있다. Focused repair entry point는 결제 내역의 `repair=required` 모드이며, writer는 여기서 누락 필수값을 채워 `ImportIssue`를 committed `Transaction`으로 승격하거나, ambiguous duplicate를 제외/새 거래 등록으로 처리한다.
4. 인라인 편집/카테고리 변경/롤백/소프트 삭제는 `ReviewsController#update_transaction`, `ReviewsController#bulk_update`, 거래 컨트롤러의 인라인 액션으로 가능.
5. 중복 결정:
   - `DuplicateConfirmationsController#update` — 단일.
   - `ReviewsController#bulk_resolve_duplicates` — 일괄 (`keep_both` / `keep_original` / `keep_new`).
6. `ReviewsController#commit`:
...
7. `ReviewsController#rollback` — 이미 커밋된 세션을 되돌림. `keep_new`로 soft delete된 원본은 `restore!`로 복구.
8. `ReviewsController#discard` — pending import 세션을 폐기. 같은 세션에서 이미 auto-committed 된 거래는 `rolled_back`으로 되돌리고, 남은 `pending_review` 거래는 destroy한다.
...
- 감지: `ImportDuplicatePolicy`가 먼저 같은 import 세션 exact duplicate를 확인하고, 이후 `DuplicateDetector.new(workspace, transaction).find_match`로 기존 committed 거래와 비교한다.
- 적용: exact duplicate는 새 staging row를 삭제해 skip한다. Ambiguous duplicate는 새 staging row를 삭제하고 `ImportIssue(issue_type: "ambiguous_duplicate", duplicate_transaction_id: <기존 거래>)`로 보관한다.
- Legacy 결정: 기존 review flow와 오래된 pending duplicate에는 `DuplicateConfirmation.status` ∈ {`pending`, `keep_both`, `keep_original`, `keep_new`}가 남아 있다. 새 text/image import duplicate policy는 `DuplicateConfirmation`을 만들지 않는다. 중복 필터/달력 배지는 pending `DuplicateConfirmation`과 open `ImportIssue(issue_type: "ambiguous_duplicate")`를 합산한다.
...
| Vision은 성공했고 incomplete row만 있음 | `ImportIssue(issue_type: "missing_required_fields")`를 저장한 뒤 outcome 있음으로 처리 → 세션/파일은 completed, `parsing_complete`와 `import_repair_needed` 알림을 발송, repair record는 유지. 입력 기록과 결제 내역 repair filter는 open `ImportIssue` 요약을 보여준다. |
| Text/image parsed row가 ambiguous duplicate임 | 새 `Transaction` staging row는 삭제하고 `ImportIssue(issue_type: "ambiguous_duplicate", duplicate_transaction_id: <기존 거래>)`로 보관. 세션은 completed + repair needed로 노출된다. |
| 사용자가 repair mode에서 누락값을 모두 채움 | `ImportIssuesController#update`가 `ImportIssueResolutionService`를 호출한다. Exact duplicate면 issue를 dismissed 처리하고 거래를 만들지 않는다. Ambiguous duplicate면 같은 issue를 `issue_type: ambiguous_duplicate`로 전환한다. 중복이 없으면 committed `Transaction`을 만들고 `resolved_transaction_id`를 연결한 뒤 예산 알림을 검사한다. |
| 사용자가 ambiguous duplicate를 처리함 | "기존 유지"는 issue를 dismissed 처리한다. "새 거래 등록"은 사용자의 명시 선택으로 committed `Transaction`을 만들고 issue를 resolved 처리한다. |
...
- **파싱 완료 알림** — 잡이 `parsing_session.complete!`을 호출한 뒤 `Notification.create_parsing_complete!`이 발송된다. `AiTextParsingJob`과 `FileParsingJob`은 stats > 0이고 저장·중복·repair outcome이 있으면 complete 처리한다. 남은 `pending_review` 거래가 있으면 알림 action은 legacy review로 향하고, 자동 등록할 row가 모두 처리됐으면 결제 내역(`/transactions`)으로 향한다. Repair issue만 있으면 수정 필요한 항목 수를, exact duplicate만 있어 `success_count`가 0이면 중복 항목을 건너뛰었다는 메시지를 보낸다. 검토 단계의 `ReviewsController#commit` / `ParsingSession#commit_all!`는 알림을 만들지 않는다. 수신자는 워크스페이스 owner + 멤버 (`co_owner` / `member_write` / `member_read`).
...
- **가져오기 수정 필요 알림** — `AiTextParsingJob`과 `FileParsingJob`은 open `ImportIssue`가 있는 세션에 대해 `Notification.create_import_repair_needed!`를 best-effort로 호출한다. 수신자는 owner + `co_owner` / `member_write`이며, `action_url`은 `/workspaces/:id/transactions?repair=required&import_session_id=:session_id`로 해당 import의 repair item만 보여준다. 생성 시 사용자 Turbo stream의 `flash` target으로 warning toast도 broadcast된다. 결제 내역 기본 화면에는 전체 open repair count banner가 표시되고, `repair=required` 모드에서는 repair item만 보인다. 마지막 open repair item이 resolved/dismissed 되면 해당 세션의 unread `import_repair_needed` 알림은 읽음 처리된다.
