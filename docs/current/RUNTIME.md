...
5. 결과가 0건이거나 저장·중복·repair issue 중 어떤 outcome도 없으면 `fail!` + `create_failure_notifications`.
...
7. 통계 hash로 분기:
   ...
   - `stats[:total].zero?` 또는 outcome이 전혀 없음 → count를 세션에 저장한 뒤 `parsing_session.fail!` + `processed_file.mark_failed!` + `create_failure_notifications`.
