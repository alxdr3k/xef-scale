...
텍스트 붙여넣기 ─► AiTextParsingJob ─► AiTextParser (Gemini Flash) ─┐
                                                                    ├─► 현재: pending_review Transaction ─► review ─► committed
...
전환 목표: [ADR-0001](../decisions/ADR-0001-auto-post-imports.md)은 텍스트/이미지 파싱 결과 중 complete row를 파싱 완료 시점에 자동 `committed`로 저장하고, 필수값 누락·애매한 중복만 repair queue로 보내도록 결정했다. 아래 §2.3/§3.3 review 흐름은 현재 코드의 상태이며, `INP-1B`/`UX-1B` slice가 완료되면 auto-post + repair flow로 대체된다.
...
5. 모든 거래 처리 후 통계 hash로 `parsing_session.complete!(stats)` 또는 결과/성공이 0이면 `fail!` + `create_failure_notifications`.
...
2. `parse_file`:
   ...
   - 날짜/가맹점/금액 중 필수 정보가 부족한 행은 `incomplete_transactions`로 보존한다. 현재 코드는 이 행을 `Transaction`으로 만들지 않고 `ParsingSession#error_count`와 `notes`에 기록해 검토 화면에서 사용자가 볼 수 있게 한다. P1 target은 이 임시 notes 기록을 durable repair record로 바꾸는 것이다 (`INP-1B.3`).
...
7. 통계 hash로 분기:
   - `stats[:total] > 0`이고 저장 성공이 1건 이상이면 `parsing_session.complete!(stats)` + `processed_file.mark_completed!` + `create_completion_notifications`.
   - `stats[:total].zero?` 또는 `stats[:success].zero?` → count를 세션에 저장한 뒤 `parsing_session.fail!` + `processed_file.mark_failed!` + `create_failure_notifications`.
...
4. `ReviewsController#commit`:
   ...
   - 예산 알림 검사 (`check_budget_alerts`): 월 예산이 있고 해당 월 지출이 80% 이상이면 `budget_warning`, 100% 이상이면 `budget_exceeded`.
...
- **파싱 완료 알림** — 잡이 `parsing_session.complete!`을 호출하는 시점에만 `Notification.create_parsing_complete!`이 발송된다. `AiTextParsingJob`은 stats > 0일 때, `FileParsingJob`은 stats > 0일 때. 검토 단계의 `ReviewsController#commit` / `ParsingSession#commit_all!`는 알림을 만들지 않는다. 수신자는 워크스페이스 owner + 멤버 (`co_owner` / `member_write` / `member_read`).
...
- **예산 알림** — 검토 세션 commit 후 영향을 받은 거래 월별로 `Notification.create_budget_alert!`을 호출한다. 80% 이상 100% 미만은 `budget_warning`, 100% 이상은 `budget_exceeded`이며, `workspace_id/user_id/notification_type/target_year/target_month` 조합으로 같은 사용자·월·타입의 중복 알림을 막는다.
