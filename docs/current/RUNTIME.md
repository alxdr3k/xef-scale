...
- Legacy 결정: 기존 review flow와 오래된 pending duplicate에는 `DuplicateConfirmation.status` ∈ {`pending`, `keep_both`, `keep_original`, `keep_new`}가 남아 있다. 새 text/image import duplicate policy는 `DuplicateConfirmation`을 만들지 않는다.
