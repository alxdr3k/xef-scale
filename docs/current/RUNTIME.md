...
전환 목표: [ADR-0001](../decisions/ADR-0001-auto-post-imports.md)은 텍스트/이미지 파싱 결과 중 complete row를 파싱 완료 시점에 자동 `committed`로 저장하고, 필수값 누락·애매한 중복만 repair queue로 보내도록 결정했다. 현재 코드는 complete row 자동 커밋, exact duplicate skip, ambiguous duplicate `ImportIssue(status: open)`, incomplete image row `ImportIssue(status: open)` 저장을 구현했다.
...
1. 사용자가 `/workspaces/:id/parsing_sessions/:id/review` 페이지를 열어 아직 pending으로 남은 전환기 거래를 확인할 수 있다. 새 text/image import duplicate policy는 `DuplicateConfirmation`을 만들지 않지만, legacy 데이터와 일부 non-complete staging row를 위해 review route는 아직 남아 있다. Focused repair entry point는 결제 내역의 `repair=required` 모드에 있으며, inline editing/promotion은 `UX-1B.3`/`INP-1B.4`에서 완성한다.
