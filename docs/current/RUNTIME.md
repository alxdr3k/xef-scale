...
4. 각 거래 생성 후 `ImportDuplicatePolicy`가 중복을 처리한다.
   - 같은 import 세션 안의 exact duplicate 또는 기존 committed 거래와 date/amount/normalized merchant/installment가 같은 exact duplicate는 staging row를 삭제하고 `duplicate_count`만 증가시킨다.
