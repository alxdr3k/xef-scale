...
7. 자동 커밋된 거래 월에 대해 `BudgetAlertService.create_for_transactions!`가 예산 알림을 생성하고, `parsing_session.complete!(stats)` 후 `Notification.create_parsing_complete!`로 워크스페이스 owner + (`co_owner` / `member_write` / `member_read`)에게 알림. Open `ImportIssue`가 있으면 owner + `co_owner` / `member_write`에게 repair 알림도 보낸다. 이 post-commit side effect는 best-effort이며, 실패해도 이미 커밋된 거래와 completed 세션을 failed로 되돌리지 않는다. 사용자가 입력 기록에서 해당 import를 되돌리면 committed row는 `rolled_back`이 되고 open repair 알림은 read 처리된다.
...
1. 입력 기록(`/workspaces/:id/parsing_sessions`)의 writer-only `되돌리기` 액션은 `ParsingSessionsController#undo` → `ParsingSession#undo_import!`를 호출한다. 같은 import의 committed row는 `rolled_back`, pending row는 destroy, open `ImportIssue`는 `dismissed`로 전환하고 repair notification은 read 처리한다. committed row가 하나라도 되돌아가면 세션은 `review_status: rolled_back`, repair/pending-only import는 `discarded`가 된다.
2. 사용자가 `/workspaces/:id/parsing_sessions/:id/review` 페이지를 열어 아직 pending으로 남은 전환기 거래를 확인할 수 있다. 새 text/image import duplicate policy는 `DuplicateConfirmation`을 만들지 않지만, legacy 데이터와 일부 non-complete staging row를 위해 review route는 아직 남아 있다. Focused repair entry point는 결제 내역의 `repair=required` 모드이며, writer는 여기서 누락 필수값을 채워 `ImportIssue`를 committed `Transaction`으로 승격하거나, ambiguous duplicate를 제외/새 거래 등록으로 처리한다.
3. 인라인 편집/카테고리 변경/롤백/소프트 삭제는 `ReviewsController#update_transaction`, `ReviewsController#bulk_update`, 거래 컨트롤러의 인라인 액션으로 가능.
4. 중복 결정:
   - `DuplicateConfirmationsController#update` — 단일.
   - `ReviewsController#bulk_resolve_duplicates` — 일괄 (`keep_both` / `keep_original` / `keep_new`).
5. `ReviewsController#commit`:
...
6. `ReviewsController#rollback` — 이미 커밋된 세션을 되돌림. `keep_new`로 soft delete된 원본은 `restore!`로 복구.
7. `ReviewsController#discard` — pending import 세션을 폐기. 같은 세션에서 이미 auto-committed 된 거래는 `rolled_back`으로 되돌리고, 남은 `pending_review` 거래는 destroy한다.
