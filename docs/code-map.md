...
| [app/controllers/reviews_controller.rb](../app/controllers/reviews_controller.rb) | 검토함 인덱스(`index`, ADR-0004 — `needs_review` + cross-tenant safe duplicates)·파싱 세션 검토(`show`)·커밋·롤백·중복 일괄 해결. `set_parsing_session` / `require_workspace_write_access`는 인덱스에서 except. | implemented |
...
| [app/controllers/categories_controller.rb](../app/controllers/categories_controller.rb) | 카테고리 CRUD + 학습된 매핑 미리보기 (Phase 3.4 결합). 슬라이드오버 거래 적용 시 ADR-0011 manual_set 기록. | implemented |
...
| [app/controllers/category_learning_suggestions_controller.rb](../app/controllers/category_learning_suggestions_controller.rb) | 인라인 카테고리 학습 explicit opt-in endpoint (ADR-0007 §4) — admin-only, transaction merchant 기반 manual exact 매핑 생성 | implemented |
| [app/controllers/workspaces_controller.rb](../app/controllers/workspaces_controller.rb) | 워크스페이스 CRUD + AI 동의 처리 | implemented |
| [app/controllers/workspace_more_controller.rb](../app/controllers/workspace_more_controller.rb) | 더보기 전용 페이지 (Phase 3.5) — 그룹 리스트 카드 진입점만 모은다. 세부 화면은 기존 컨트롤러 사용. | implemented |
...
| [app/models/transaction.rb](../app/models/transaction.rb) | 거래 — status 머신, payment_type, source_type, classification_source (ADR-0011, 결정 메커니즘 4값, nullable), soft delete | implemented |
...
| Path | Purpose |
|------|---------|
| [app/views/layouts/application.html.erb](../app/views/layouts/application.html.erb) | 최상위 layout — `<head>` 메타/CSS/JS 링크, 본문은 `shared/_screen_shell` 위임 |
| [app/views/shared/_screen_shell.html.erb](../app/views/shared/_screen_shell.html.erb) | body 골조 (ADR-0003 Phase 1) — `bg-page` 토큰, navbar/mobile_bottom_nav/flash 슬롯 조립 |
| [app/views/layouts/_navbar.html.erb](../app/views/layouts/_navbar.html.erb) | 데스크탑 nav — 5탭 IA (ADR-0004): 홈/거래/검토함/카테고리/더보기 |
| [app/views/layouts/_mobile_bottom_nav.html.erb](../app/views/layouts/_mobile_bottom_nav.html.erb) | 모바일 하단 nav — 5탭 IA (ADR-0004) |
| [app/views/reviews/index.html.erb](../app/views/reviews/index.html.erb) | 검토함 인덱스 — 세그먼트 탭 골조(파싱 결과 / 중복 후보) + "+ 새로 가져오기" input_sheet trigger. 행 디자인은 후속 화면 PR. |
| [app/views/shared/_input_sheet.html.erb](../app/views/shared/_input_sheet.html.erb) | 3-way 입력 시트 (ADR-0004, Phase 3.3) — 문자 붙여넣기 / 스크린샷 업로드 / 직접 입력 링크. 데스크탑 modal, 모바일 bottom sheet. AI 동의 inline_alert 포함. |
| [app/javascript/controllers/review_tabs_controller.js](../app/javascript/controllers/review_tabs_controller.js) | 검토함 인덱스의 세그먼트 탭 토글 (Stimulus). `shared/_segmented_tabs` 추출은 후속. |
| [app/javascript/controllers/input_sheet_controller.js](../app/javascript/controllers/input_sheet_controller.js) | input_sheet 토글 (Stimulus). Esc · 백드롭 · close 버튼으로 닫기. |
| [app/views/shared/_context_header.html.erb](../app/views/shared/_context_header.html.erb) | 화면명 + 라이브 컨텍스트 + 우측 액션 (synthesis.md 6.3 / X3) — 페이지 단위 채택은 Phase 3 |
| [app/views/shared/_hero_stat.html.erb](../app/views/shared/_hero_stat.html.erb) | 화면 최상위 큰 숫자 (X1) — 페이지 단위 채택은 Phase 4 |
| [app/views/shared/_amount.html.erb](../app/views/shared/_amount.html.erb) | 금액 단일 표기 — tabular-nums + 시맨틱 톤(neutral/positive/warning/danger/muted) |
| [app/views/shared/_source_icon.html.erb](../app/views/shared/_source_icon.html.erb) | 거래 입력 경로 아이콘 (ADR-0003 Phase 2) — manual/text_paste/image_upload/api/import. 채택은 `_transaction_row` 통합 시. |
| [app/views/shared/_pending_badge.html.erb](../app/views/shared/_pending_badge.html.erb) | pending_review 상태 도트 — text-warning. 채택은 `_transaction_row` 통합 시. |
| [app/views/shared/_category_source_chip.html.erb](../app/views/shared/_category_source_chip.html.erb) | 카테고리 chip + 결정 메커니즘 마크 (ADR-0007) — mapping/keyword/gemini/manual. 채택은 `_transaction_row` 통합 시. |
| [app/views/shared/_inline_alert.html.erb](../app/views/shared/_inline_alert.html.erb) | 화면 인라인 알림/제안 (ADR-0007 §4 학습 제안, ui-redesign-plan §4.1 AI 동의) — info/warning/positive/danger/ai 톤, actions 슬롯. 채택은 Phase 2~. flash와 다른 컴포넌트 — 정적 인라인. |
...
| [app/views/transactions/_transaction_row.html.erb](../app/views/transactions/_transaction_row.html.erb) | 단일 거래 row partial — transactions/index, reviews/show 양쪽 사용. `@parsing_session` 유무로 update_url 자동 분기, `read_only`/`categories` 로컬 수용 (Phase 2 통합) |
| [app/views/dashboards/](../app/views/dashboards/) | 캘린더/월별/연별/반복결제 대시보드 |
| [app/javascript/](../app/javascript/) | Stimulus 컨트롤러 |
| [app/assets/stylesheets/application.tailwind.css](../app/assets/stylesheets/application.tailwind.css) | Tailwind 4 엔트리 — `@theme` 토큰 + `@import "./pretendard-dynamic-subset.css"` |
| [app/assets/stylesheets/pretendard-dynamic-subset.css](../app/assets/stylesheets/pretendard-dynamic-subset.css) | Pretendard Variable 92개 `@font-face` (unicode-range) — ADR-0010 |
| [public/fonts/pretendard-1.3.9/](../public/fonts/pretendard-1.3.9/) | Pretendard Variable WOFF2 92개 subset (자가 호스팅, P-public) |
| [public/licenses/pretendard-OFL-1.1.txt](../public/licenses/pretendard-OFL-1.1.txt) | Pretendard SIL OFL 1.1 라이선스 고지 |
