...
E2E는 별도로 `e2e/` 디렉토리에 Playwright 스펙으로 존재한다.
...
- CI에서 자동 실행되지 않는다 (현재 워크플로 미연결). 본 PR은 이 사실을 변경하지 않는다.
...
`GEMINI_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `RAILS_MASTER_KEY` 등 환경변수는 `docs/operations.md` 참조.
...
- [ ] `bin/brakeman --no-pager`
- [ ] `bin/bundler-audit`
- [ ] (UI/플로우 변경 시) `bunx playwright test`로 관련 스펙 확인
