...
<!-- my-skill:generated
skill: dev-cycle
base-sha256: ae78c72f8316498767261b2e7702de528b7121008d887fc7f077291a9f52af78
overlay-sha256: e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
output-sha256: ae78c72f8316498767261b2e7702de528b7121008d887fc7f077291a9f52af78
...
필수 필드: `schema_version`, `cycle`, `result`, `actions`, `conclusion.summary_ko`, `verification`, `review_ship`, `risks`. `cycle`은 1부터 시작하는 정수이고, `actions`와 `verification`은 비어 있으면 안 되며 각 항목에 사용자-visible `summary_ko`를 쓴다. 권장 `result` 값은 `shipped`, `blocked`, `all_clear`, `doc_fix_needed`다. 리스크가 없으면 `risks: []`를 쓴다. Ready leaf가 없어 `ALL CLEAR`로 끝낼 때는 실제 수행한 탐색/판단을 `actions`와 `conclusion`에 쓰고, ready가 아닌 다음 검토 후보는 최대 3개까지 `next_candidates`에만 둔다. 후보는 후속 안내이지 risk issue 대상이 아니므로 실제 리스크가 없으면 `risks: []`다.

`finish-cycle-json` stdout은 tool output일 뿐 사용자에게 자동 전달되지 않는다. stdout ack JSON에서 `rendered_markdown`만 추출해 사용자에게 그대로 보여준다. 이 메시지가 사용자에게 보이기 전에는 다음 `update_plan`, Step 1, Step 2, discovery, 파일 탐색, 또는 tool call을 하지 않는다. 한 줄짜리 "사이클 N 완료" 요약으로 대체하면 안 된다. `--loop` 또는 `--loop N`이면 이 user-visible brief를 보낸 뒤에만 다음 cycle로 간다.
...
- **ALL CLEAR**: `result:"all_clear"` payload로 `finish-cycle-json`을 실행한 뒤 종료한다. Ready leaf가 없어서 종료하는 경우, 실제 탐색 행동과 결론은 `actions`/`conclusion`에 쓰고 Step 2의 다음 검토 후보는 `next_candidates`에만 포함한다. 실제 리스크가 없으면 `risks: []`다.
