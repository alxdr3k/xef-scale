...
Rails 기반 지출 추적 앱. Excel/PDF 명세서 자동 파싱이나 이메일 연동, 로컬 디렉토리 감시형 파서 같은 구조는 사용하지 않는다. 사용자는 다음 세 가지 경로로만 거래를 입력한다: 직접 입력, 금융 문자 붙여넣기, 명세서 스크린샷 업로드.

## 입력 경로

1. **직접 입력 (수동)**
   - `TransactionsController#new/#create`가 웹 UI에서 거래를 받아 즉시 `committed` 상태로 저장한다.
   - 파싱 세션/검토 흐름을 거치지 않는다.
...
- 위 세 가지(직접 입력, 문자 붙여넣기, 이미지 업로드) 외의 입력 경로는 이 제품 범위가 아니다.
- **광고 청정**: 가계부 본면(홈/거래/검토함/카테고리)에 외부 금융상품 추천·광고를 노출하지 않는다. 자세한 정책과 supersede 조건은 [ADR-0005](docs/decisions/ADR-0005-ad-free-policy.md).
