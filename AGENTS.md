...
| 제품 스코프 | [PRD.md](PRD.md) |
| 현재 구현 요약 | [docs/context/current-state.md](docs/context/current-state.md) 및 `docs/runtime.md`, `docs/data-model.md`, `docs/ai-pipeline.md`, `docs/categorization.md`, `docs/architecture.md`, `docs/code-map.md` |
| 수락된 결정의 이유 | [docs/decisions/](docs/decisions/) |
| 테스트/린트/스캔 명령 | [docs/testing.md](docs/testing.md) |
| 운영/배포 | [docs/operations.md](docs/operations.md) (Claude 전용 안전 규칙은 [CLAUDE.md](CLAUDE.md)) |
...
- AI/파서/카테고리화 동작이 바뀌면 [docs/ai-pipeline.md](docs/ai-pipeline.md) 또는 [docs/categorization.md](docs/categorization.md) 갱신.
...
- 명령은 [docs/testing.md](docs/testing.md)에서 가져온다. 새로 만들지 않는다.
...
Excel/PDF/CSV/HTML 명세서, 이메일/IMAP, 크롤러, 디렉토리 워처는 **스코프 밖**이다. 자세히는 [PRD.md](PRD.md).

## 카테고리화 (현재)

이미지 경로만 Gemini 카테고리 폴백을 호출한다. 텍스트 경로는 `CategoryMapping` + `Category#keyword`까지만. 이를 변경하려면 ADR 필요. 자세히는 [docs/categorization.md](docs/categorization.md), [docs/ai-pipeline.md](docs/ai-pipeline.md).
