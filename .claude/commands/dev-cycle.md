...
- **ALL CLEAR**: `result:"all_clear"` payload로 `finish-cycle-json`을 실행한 뒤 종료한다. Ready leaf가 없어서 종료하는 경우, 실제 탐색 행동과 결론은 `actions`/`conclusion`에 쓰고 Step 2의 `Next review candidates`는 `next_candidates`에만 포함한다. 실제 리스크가 없으면 `risks: []`다.
