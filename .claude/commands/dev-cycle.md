...
- `--phase <id>` : 지정한 Phase 완료 후 즉시 중단. id는 Phase 번호 또는 식별자 (`1`, `3`, `4`, `6` 등).

`$ARGUMENTS` 파싱 규칙:
- `--loop` 단독: 무한 반복 (ALL CLEAR 시 종료)
- `--loop N` (N은 정수): N회 반복 후 종료
- `--phase <id>`: 해당 Phase 완료 직후 중단 (loop와 함께 쓰면 매 사이클마다 해당 Phase까지만 실행)
- 플래그 없음: 단일 전체 실행

**`--phase` 중단 체크:** 각 Phase 완료 시점에 `--phase` 값과 현재 Phase 번호를 비교한다. 일치하면 그 자리에서 중단하고 사용자에게 보고한다.
...
> `--phase 6` 이면 리뷰 pass 후 중단 (Phase 7 실행하지 않음).
