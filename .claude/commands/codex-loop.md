...
- **Pass 신호** (아래 둘 중 하나만으로도 Pass):
  ...
  - baseline 이후 `$CODEX_PASS_ACTOR`가 남긴 `issue_comment` 또는 `review` body가 `$CODEX_PASS_COMMENT_PATTERN`을 case-insensitive substring으로 포함
...
- 1a 또는 1b가 > 0 → **Pass**. "Merge 처리"로 진행 후 `mcp__github__unsubscribe_pr_activity` 호출.
...
Path A의 Pass 신호(reaction 또는 pass-comment 매칭) 감지 또는 Path B의 exit 0은 Codex pass를 의미한다. 사용자의 추가 확인을 기다리지 말고 PR을 merge한다. 단, merge 전 다음을 확인한다.
